## 𝐇𝐞𝐥𝐥𝐨 𝐭𝐡𝐞𝐫𝐞!<img src="https://user-images.githubusercontent.com/74038190/214644152-52f47eb3-5e31-4f47-8758-05c9468d5596.gif" width="30px"></a></h1>

<br/>
 <img align="left" src="https://user-images.githubusercontent.com/74038190/212284087-bbe7e430-757e-4901-90bf-4cd2ce3e1852.gif" width="80">    
</a>
<p align="center">
<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=2000&pause=100&multiline=true&width=435&height=100&separator=%3C&lines=I+am+Vaibhav+Rai;+%3CLead+Software+Engineer+%40+Tydo;+%3CBackend+%7C+Cloud+%7C+Data-Pipeline+%7C+Devops+Expert;+%3CDistributed+Systems+%7C+Microservice%7C+Infrastructure+Expert" alt="Typing SVG" /></a>
</p>

## Online Portfolio
<p align="left" >
    <a href="https://www.linkedin.com/in/vaibhavrai46/">
    <img alt="Linkedin" src="https://img.shields.io/badge/Linkedin-0A66C2?style=for-the-badge&logo=github&logoColor=white">
<a href="https://github.com/vaibhav2408">
    <img alt="GitHub" src="https://img.shields.io/badge/Github-181717?style=for-the-badge&logo=github&logoColor=white">
  </a>
<a href="mailto:vaibhavrai46@gmail.com">
    <img alt="Gmail" src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=Gmail&logoColor=white">
  </a>
<a href="https://www.linkedin.com/in/vaibhavrai46/">
    <img alt="Resume" src="https://img.shields.io/badge/Resume-FFFF66?style=for-the-badge&logo=adobeacrobatreader&logoColor=black">
  </a>
</p>


# Hi there, I'm Vaibhav Rai 👋

## Lead Software Engineer | Data Engineering & AI/ML Expert

🚀 **8+ years** of experience building scalable backend and data platforms and AI-powered solutions.
💼 Currently leading engineering at **Tydo** (US Remote)
🎯 Specialized in **Backend Engineering**, **Data Engineering**, **Microservices Architecture**, **AI/ML Integration**, and **DevOps**

---

## 🔥 Highlights

- 💰 Built AI-powered Klaviyo campaign platform generating **$1M+ in attributable revenue**
- 🤖 Designed **Model Context Protocol (MCP)** server enabling intelligent e-commerce data querying
- 📊 Architected data pipelines processing **5TB+ daily** with <0.1% error rate serving **4,000+ stores**
- ⚡ Engineered real-time metrics API handling **1M+ daily requests** with sub-500ms latency
- 🏆 Multiple awards: Engineering Impact Award, Employee of the Quarter, HPE Spot Awards

---

## 💻 Technical Expertise

### Languages
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/-Java-007396?style=flat-square&logo=java&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)

### Backend & APIs
![Django](https://img.shields.io/badge/-Django-092E20?style=flat-square&logo=django&logoColor=white)
![FastAPI](https://img.shields.io/badge/-FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![Flask](https://img.shields.io/badge/-Flask-000000?style=flat-square&logo=flask&logoColor=white)
![GraphQL](https://img.shields.io/badge/-GraphQL-E10098?style=flat-square&logo=graphql&logoColor=white)

### Cloud Platforms
![AWS](https://img.shields.io/badge/-AWS-232F3E?style=flat-square&logo=amazon-aws&logoColor=white)
![GCP](https://img.shields.io/badge/-Google%20Cloud-4285F4?style=flat-square&logo=google-cloud&logoColor=white)

### Databases & Storage
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/-MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![BigQuery](https://img.shields.io/badge/-BigQuery-4285F4?style=flat-square&logo=google-cloud&logoColor=white)
![Elasticsearch](https://img.shields.io/badge/-Elasticsearch-005571?style=flat-square&logo=elasticsearch&logoColor=white)
![Cassandra](https://img.shields.io/badge/-Cassandra-1287B1?style=flat-square&logo=apache-cassandra&logoColor=white)
![DynamoDB](https://img.shields.io/badge/-DynamoDB-4053D6?style=flat-square&logo=amazon-dynamodb&logoColor=white)

### Data Engineering
![Airflow](https://img.shields.io/badge/-Airflow-017CEE?style=flat-square&logo=apache-airflow&logoColor=white)
![DBT](https://img.shields.io/badge/-DBT-FF694B?style=flat-square&logo=dbt&logoColor=white)
![Kafka](https://img.shields.io/badge/-Kafka-231F20?style=flat-square&logo=apache-kafka&logoColor=white)
![Airbyte](https://img.shields.io/badge/-Airbyte-615EFF?style=flat-square&logo=airbyte&logoColor=white)
![PubSub](https://img.shields.io/badge/-PubSub-231F20?style=flat-square&logo=apache-kafka&logoColor=white)

### AI/ML Integration
- **LLMs** | **Model Context Protocol (MCP)** | **Claude Code** | **Prompt Engineering**

### DevOps & Tools
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/-Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Terraform](https://img.shields.io/badge/-Terraform-7B42BC?style=flat-square&logo=terraform&logoColor=white)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/-GitHub%20Actions-2088FF?style=flat-square&logo=github-actions&logoColor=white)
![CirlceCI](https://img.shields.io/badge/-CirlceCI-326CE5?style=flat-square&logo=kubernetes&logoColor=white)

### Monitoring & Analytics
![Grafana](https://img.shields.io/badge/-Grafana-F46800?style=flat-square&logo=grafana&logoColor=white)
![Prometheus](https://img.shields.io/badge/-Prometheus-E6522C?style=flat-square&logo=prometheus&logoColor=white)
![Datadog](https://img.shields.io/badge/-Datadog-632CA6?style=flat-square&logo=datadog&logoColor=white)
![New Relic](https://img.shields.io/badge/-New%20Relic-008C99?style=flat-square&logo=new-relic&logoColor=white)

---

## 🚀 Professional Journey

### **Lead Software Engineer** @ [Tydo](https://ltv.ai) *(Oct 2022 - Present)*
- Built AI-powered marketing automation platform with custom segmentation algorithms
- Designed MCP server for intelligent e-commerce data querying via Slack
- Architected scalable data pipeline API ecosystem processing 5TB+ daily
- Developed high-performance integration framework securing $240K+ ARR
- Built self-serve onboarding portal increasing trial-to-paid conversion by 35%

### **Lead Software Engineer** @ Cerebra *(Aug 2021 - Oct 2022)*
- Developed e-commerce apps for Shopify and BigCommerce app stores
- Built event-driven email service improving customer engagement
- Led technical due diligence during company acquisition

### **Senior Software Engineer** @ Hewlett Packard (HPE) *(Nov 2020 - Aug 2021)*
- Led development of multi-tenant RBAC audit-trail system for HPE GreenLake
- Improved Aruba Central audit system performance by 50%

### **Software Engineer** @ Bidgely *(May 2017 - Nov 2020)*
- Developed scalable PDF reporting system generating 20M+ reports daily
- Implemented electricity disaggregation ML module API endpoints

---

## 🎓 Education

**Bachelor of Engineering, Computer Science**
Visvesvaraya Technological University (VTU) - Bangalore
*GPA: 4.3/5 (Distinction)* | *Graduated: May 2017*

---

## 🏆 Awards & Recognition

- 🥇 **Engineering Impact Award** - Klaviyo campaign framework (Tydo)
- 🌟 **Employee of the Quarter** - API marketplace implementation (Cerebra)
- ⭐ **Multiple HPE Spot Awards** - Audit-trail API development
- 🏅 **Bidgely Hackathon Winner** - API performance optimization

---

## 📫 Let's Connect

[![Email](https://img.shields.io/badge/-Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:vaibhavrai46@gmail.com)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/vaibhavrai46/)
[![Phone](https://img.shields.io/badge/-Phone-25D366?style=flat-square&logo=whatsapp&logoColor=white)](tel:+918904603043)

---

## Couple of Fun Facts
- 🏏 Club level cricket player
- 🤓 Self-proclaimed coding nerd who loves diving deep into complex technical challenges



<p align="center">
<img alt="github-snake" src="https://raw.githubusercontent.com/Sutil/Sutil/2b2fad3bf54522bb30c8c170591fc68ff51b69e6/github-contribution-grid-snake2.svg" title="🐍 Watch how the snake's eating my contributions" />
</p>

---
